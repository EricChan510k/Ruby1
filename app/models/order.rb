...
class Order < ActiveRecord::Base
  ...
  PRICE = (Rails.env == 'production' ? 30000 : 30) # in cents
  DISCOUNT_AMT = (Rails.env == 'production' ? 5000 : 5) # in cents
  DISCOUNT_INCR = 4
  ...
  has_many :order_submissions,    :dependent => :destroy, :uniq => true
  has_many :submissions, :through => :order_submissions, :uniq => true
  ...
  before_save :generate_prices


  def self.active_merchant_mapped_keys; {:brand => :cc_brand, :number => :card_number, :year => :exp_year, :month => :exp_month, :verification_value => :cvv_number}; end


  def complete(cvv_number)
    payment_transaction = self.build_payment
    payment_transaction.user_id = self.user_id
    payment_transaction.ccard_last4 = nil #credit_card.last_digits
    payment_transaction.first_name = self.shipping_first_name
    payment_transaction.last_name = self.shipping_last_name
    payment_transaction.price_subtotal = self.calculate_price.fractional + self.discount_amount.fractional
    payment_transaction.price_discount = self.discount_amount.fractional
    payment_transaction.price_total = self.calculate_price.fractional
  ...
  def completed?; !self.token_key.blank?; end
  ...
  def discount_amount
    ct = self.submissions.map{|s| s.categories.count}.inject(:+) || 0
    Money.new(ct > DISCOUNT_INCR ? (DISCOUNT_AMT * (ct-DISCOUNT_INCR)) : 0)
  end

  def calculate_price
    ct = self.submissions.map{|s| s.categories.count}.inject(:+) || 0
    Money.new(ct * PRICE) - self.discount_amount
  end
...
private
  ...
  def generate_prices
    self.balance_amount = self.calculate_price.fractional
  end
