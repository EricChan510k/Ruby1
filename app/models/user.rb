...
class User < ActiveRecord::Base
  ...
  after_create :deliver_user_welcome

  has_many :submissions
  has_many :editing_submissions, :class_name=>"Submission", :conditions => ['submissions.status IN(?)', ['draft','whole']]
  ...
  validates :first_name, :last_name, :company_name, :occupation, presence: true, length: {maximum: 50}
  ...
  def deliver_user_welcome
    mail = UserMailer.user_welcome(self)
    mail.deliver
  end
