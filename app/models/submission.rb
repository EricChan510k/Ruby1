...
#                  :company_attributes, :contact_attributes, :pictures_attributes
  ...
  has_many :submission_categories,                  :dependent => :destroy, :uniq => true
  has_many :categories, :through => :submission_categories, :uniq => true
  ...
  scope :editing_submissions, where( :status => [STATUS_KINDS[:draft], STATUS_KINDS[:whole]])
