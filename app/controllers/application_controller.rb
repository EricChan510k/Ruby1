class ApplicationController < ActionController::Base
  ...
  def app_init
    # puts Payment.all.last.deliver_payment_receipt.body
  end
    ...
    def notify_error_service(e)
      ...
      # Errplane.transmit(e) rescue nil
		...
		def current_submission
			@current_submission ||= current_user.submissions.find_by_id(session[:submission_id]) unless session[:submission_id].nil?
		end
    
    def current_order
      @current_order ||= Order.incompleted.find_by_id(session[:order_id]) unless session[:order_id].nil?
    end
